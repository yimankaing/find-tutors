...
export default {
  name: "Subjects",
  components: { TableToolbar, TableAction, subjectNew },
  ...
  data() {
    return {
      ...
      tableTitles: [
        ...
        { label: "Colors", prop: "colors" }
      ],
      tableFilters: [
        {
          prop: ["_id", "khName", "enName"],
          value: ""
        }
      ],
      // Modal
  ...
  methods: {
    ...
    edit(row) {
      ...
      let data = {
        ...
        enName: row.enName,
        path: row.path,
        firstColor: row.colors.firstColor,
        secondColor: row.colors.secondColor
      };

      this.updateDoc = data;
      this.currentModal = subjectEdit;
      this.$nextTick(() => {
        this.modalVisible = true;
      });
    },
